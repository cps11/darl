...
class Relation(nn.Module):
    ...
    def forward(self, class_vector, query_encoder):  # (C,H) (Q,H)
        ...
        probs = torch.mm(V, self.W) + self.b
        return probs
...
class InductionNet(nn.Module):
    ...
    def fit(self, x):
        support_encoder, _ = self.encoder(x)  # (k*c, 2*hidden_size)
        class_vector = self.induction(support_encoder)
        probs = self.relation(class_vector, support_encoder)
        return probs

class Inductionlm(InductionNet):
    def __init__(self, C, S, weights, vocabulary):
        self.vocabulary = vocabulary
        super().__init__(C, S, len(self.vocabulary), 300, 128, 64, 3, 100, weights=weights)
    
    def criterion(self, logits, target):
        pred = torch.sigmoid(logits)
        target_onehot = torch.zeros_like(pred)
        target_onehot = target_onehot.scatter(1, target.reshape(-1, 1), 1)
        loss = torch.mean((pred - target_onehot) ** 2)

        return loss

    def forward(self, x, y):
        pred = super().forward(x)
        target = y[self.induction.C*self.induction.S:]
    ...
    def fit(self, x):
        with torch.no_grad():
            ...
            return torch.nn.functional.log_softmax(probs, dim=-1)